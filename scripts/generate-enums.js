		...
		download(file.download_url, function(fileContents) {
			...
			fileContents.split('\n').forEach(function(line) {
				...
				if (!currentEnum) {
					...
					if (line.match(/^};?$/)) {
						process.stdout.write(`Generating ${currentEnum.name}.ts... `);
						...
						writeEnumFile(currentEnum.name, currentEnum.values);
...
function processProtobufEnums() {
	...
	for (let enumName in Schema) {
		...
		process.stdout.write(`Generating ${enumName}.ts... `);
		...
		writeEnumFile(enumName, processed);
 ...
 * @param {{name: string, value: number, comment?: string}[]} enumCases
 */
function writeEnumFile(enumName, enumCases) {
	// Rename any hardcoded-bad cases
	enumCases.forEach((enumCase) => {
		if (enumCase.name == '16BitLittleEndian') {
			enumCase.name = 'SixteenBitLittleEndian';
		}
	});

	// Filter out any cases that are a duplicate name with a different value
	enumCases = enumCases.filter(enumCase => !(
		enumCase.comment?.trim().startsWith('removed')
		&& enumCases.some(c => c.name == enumCase.name && c.value != enumCase.value)
	));

	// Filter out any cases named "OBSOLETE"
	enumCases = enumCases.filter(c => c.name != 'OBSOLETE');

	let fileContent = `${GENERATED_FILE_HEADER}enum ${enumName} {\n`;
	fileContent += enumCases.map(v => `\t${quoteObjectKey(v.name)} = ${v.value},${v.comment ? ` // ${v.comment.trim()}` : ''}`).join('\n');
	fileContent += `\n}\n\nexport default ${enumName};\n`;
	FS.writeFileSync(Path.resolve(__dirname, '..', 'src', 'enums', `${enumName}.ts`), fileContent);
}
